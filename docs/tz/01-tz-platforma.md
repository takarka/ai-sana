...
> **Оговорка о товарных знаках и содержании банка заданий.** PISA —
> зарегистрированный товарный знак OECD. Платформа CRAFT AI является независимым
> образовательным продуктом и не аффилирована с OECD. Решением заказчика банк
> заданий модуля CRAFT PISA формируется **только из тренировочных заданий
> собственной разработки**, созданных методистами платформы по открытым рамкам
> оценивания OECD. Лицензирование оригинальных материалов OECD и публикация
> подлинных тестовых заданий прошлых циклов в объём проекта не входят.
> В интерфейсах модуля обязателен дисклеймер (текст зафиксирован в
> `eighth-version/js/lang/pisa.js`, ключ `pisa.disclaimer`). Перечень внешних
> источников модуля, их категории и правовой статус — раздел 6.4.4.
...
- Кабинет родителя в любом виде — решением заказчика исключён из продукта
  целиком (см. FR-CORE-09). Взаимодействие с родителями остаётся зоной
  ответственности школы и ведётся вне платформы.
...
- Лицензирование и публикация подлинных заданий OECD PISA — банк формируется только
  из тренировочных заданий собственной разработки (см. 1.3, FR-PSA-01).
- Развёртывание собственной (self-hosted) языковой модели — используются внешние
  провайдеры LLM (см. PD-09 части 2, FR-AI-12).
...
| Классный руководитель | `homeroom` | Свой класс целиком (все предметы платформы) | Сводный прогресс класса, подготовка сводок для родительских собраний |
...
flowchart TB
    ...
    subgraph Ext["Внешние сервисы"]
        LLM["Внешние провайдеры LLM<br/>(единственный компонент<br/>вне контура РК)"]
        MAIL["Почта / SMS"]
...
| Notifications | Уведомления учителю и ученику | Средняя |
...
| FR-CORE-09 | — | **ИСКЛЮЧЕНО решением заказчика.** Кабинет родителя не разрабатывается: у родителей нет учётных записей, доступа к платформе и уведомлений от неё. Информирование родителей о прогрессе ребёнка выполняет школа средствами, принятыми в организации (родительские собрания, электронный журнал, выгрузки из платформы, подготовленные учителем). Номер требования сохранён для трассируемости и не переиспользуется. |
| FR-CORE-10 | M | Учёт согласий (без участия родителя в платформе): согласие законного представителя на обработку ПДн ребёнка собирается школой вне системы (бумажная или электронная форма по шаблону платформы), а администратор организации или классный руководитель фиксирует в системе факт, дату, версию текста согласия и способ получения, при необходимости прикладывая скан. Без зафиксированного действующего согласия учётная запись ученика не активируется. Отзыв согласия фиксируется тем же порядком и немедленно блокирует учётную запись. |
...
| ID | Приоритет | Требование |
|---|---|---|
| FR-PSA-01 | M | Банк **тренировочных заданий собственной разработки**, составленных методистами платформы по открытым рамкам оценивания OECD (формат стимул + связанные вопросы), с полнотекстовым поиском и фильтрами по направлению, уровню сложности, когнитивному навыку и контексту. Подлинные задания прошлых циклов PISA в банк не включаются, лицензирование материалов OECD не производится (см. раздел 1.3). Каждое задание содержит указание автора и дату разработки. |
...
| FR-PSA-05 | M | Поддержка составных заданий PISA-типа: один стимул (текст, график, набор источников) + несколько связанных вопросов разных форматов. Эталон раскладки и навигации по юниту — UI-референсы S-5 и S-6 (раздел 6.4.4). |
...
### 6.4.4. Источники, референсы и правовой статус материалов

Раздел фиксирует внешние источники, на которых строится модуль (реестр «AI 2026»,
листы «Источники» и «экстра»), и разделяет их на категории: **норматив** —
обязателен к исполнению, **рамка оценивания** — основа методики,
**UI-референс** — образец интерфейса, но не источник контента.

| № | Ресурс | Категория | Роль в модуле | Статус |
|---|---|---|---|---|
| S-1 | [PISA 2029 Media & AI Literacy](https://www.oecd.org/en/about/projects/pisa-2029-media-and-artificial-intelligence-literacy.html) — страница проекта OECD; черновик рамки — `PISA 2029 MAIL Assessment Framework First Draft.pdf` | Рамка оценивания | Основа спецмодуля «Медиа- и ИИ-грамотность» (FR-PSA-03): понимание работы медиаплатформ и ИИ-систем, этические последствия, взаимодействие с ИИ, критическая оценка контента | **First Draft.** Подлежит ревизии при выходе финальной редакции (FR-PSA-31) |
| S-2 | ИМП МП РК на 2026–2027 учебный год, направление «Цифровая грамотность и ИИ» | Норматив | Соответствие требованиям МП РК (раздел 1.3) | **Первоисточник не приложен.** В реестре указано только имя файла `проект ИМП русс.pdf` (FR-PSA-32) |
| S-3 | [Внедрение ИИ в среднее образование — МП РК](https://www.gov.kz/memleket/entities/edu/press/news/details/1271482?lang=ru) | Норматив (целевые показатели) | 43 цели обучения по ИИ; ориентир «не менее 80 % выпускников с базовыми ИИ-навыками к 2029 г.» — контекст для BG-2 и KPI раздела 2.3 | Действующий |
| S-4 | [PISA Test — released items OECD](https://www.oecd.org/en/about/programmes/pisa/pisa-test.html) | UI-референс | Образец подачи заданий; ссылка размещена на публичном сайте | **Материалы OECD**, в банк заданий не переносятся (FR-PSA-30) |
| S-5 | Плеер PISA 2022, юнит `MAT/MA104-CarPurchase` (`pisa2022-questions.oecd.org`) | UI-референс | Эталон раскладки «стимул + связанные вопросы» для FR-PSA-05 | **Материалы OECD**, только визуальный референс (FR-PSA-30) |
| S-6 | Плеер PISA 2025, юнит `CACERS020_FastFashion` (`pisa2025-questions.oecd.org`) | UI-референс | Обновлённая оболочка цикла 2025 — целевой ориентир интерфейса тренажёра | **Материалы OECD**, только визуальный референс (FR-PSA-30) |
| S-7 | Пример ИИ-симулятора — расшаренный диалог Gemini | Пример | Иллюстрация формата диалогового задания с ИИ | **Нестабильный источник:** ссылка живёт, пока её не отозвал автор (FR-PSA-33) |

| ID | Приоритет | Требование |
|---|---|---|
| FR-PSA-30 | M | Источники S-4…S-6 используются **исключительно как референсы интерфейса и формата** (раскладка «стимул + вопросы», навигация по юниту, типы полей ответа). Перенос стимулов, текстов, иллюстраций и вопросов OECD в банк заданий запрещён — банк формируется по FR-PSA-01 и разделу 1.3. Проверяется на приёмке контента: для каждого задания зафиксированы автор и дата разработки. |
| FR-PSA-31 | M | Спецмодуль FR-PSA-03 строится по рамке S-1 в редакции First Draft. При публикации финальной редакции рамки MAIL проводится ревизия состава компетенций спецмодуля и метаданных заданий (FR-PSA-06); расхождения оформляются как изменение объёма работ. |
| FR-PSA-32 | M | До старта разработки контента модуля к проекту прикладывается первоисточник ИМП (S-2) с реквизитами и датой. До этого момента утверждения о соответствии ИМП в интерфейсах и материалах публичного сайта не публикуются (связано с FR-WEB-05). |
| FR-PSA-33 | S | Внешние примеры-иллюстрации (S-7) сохраняются в проектном хранилище собственной копией (экспорт диалога либо скринкаст); ссылка на оригинал остаётся справочной. |

> **Замечание по реестру источников.** Ссылки S-4…S-6 ведут на материалы OECD и
> в исходном реестре никак не отделены от методических источников. Риск прямой:
> при наполнении банка их можно принять за разрешённый контент, что противоречит
> решению заказчика (раздел 1.3) и уже потребовало правки текстов сайта
> (FR-WEB-05). Категория ресурса в таблице выше — обязательный атрибут реестра.
...
| FR-AI-02 | M | **Деперсонализация — ключевое требование.** Языковые модели размещены у внешних провайдеров и являются единственным элементом системы за пределами контура РК (см. PD-01, PD-04 части 2). Поэтому в запрос к модели не передаются ФИО, ИИН, e-mail, телефон, наименование школы, класс, фотографии и любые иные идентифицирующие данные ученика — передаётся только псевдонимный идентификатор сессии и учебный контекст задания. Требование проверяется автотестом на каждом релизе и является блокирующим критерием приёмки. |
...
| FR-AI-05a | M | Фильтр ПДн в свободном вводе: если ученик самостоятельно вводит в промпт своё или чужое имя, адрес, телефон или иные персональные данные, ИИ-шлюз маскирует их до отправки провайдеру и показывает ученику обучающее предупреждение о недопустимости передачи персональных данных в ИИ-сервисы. |
...
| FR-AI-12 | C | Решением заказчика в v1 используются **внешние провайдеры LLM** (см. PD-09 части 2). Требование сохраняется как архитектурное ограничение: абстракция провайдера в ИИ-шлюзе должна допускать подключение self-hosted модели в контуре РК в будущем без изменения кода прикладных сервисов. Разворачивание такой модели в объём проекта не входит. |
...
| FR-NTF-04 | — | **ИСКЛЮЧЕНО решением заказчика** (следствие FR-CORE-09): платформа не отправляет уведомления родителям и не хранит их контактные данные для этой цели. |
...
| FR-WEB-05 | M | **Приведение текстов сайта в соответствие с принятыми решениями.** До запуска платформы скорректировать формулировки на публичном сайте и в политике конфиденциальности: (а) вместо «архив реальных заданий PISA прошлых лет» — тренировочные задания собственной разработки по рамкам OECD; (б) вместо безусловного «платформа не передаёт данные детей сторонним сервисам» — точная формулировка о том, что персональные данные не передаются, а обращения к ИИ выполняются в деперсонализированном виде. Требование обязательно: текущие формулировки шире того, что обеспечивает система, и создают юридический и репутационный риск (R-1). |
