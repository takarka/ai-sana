...
| ID | Требование |
|---|---|
| PD-01 | Все базы данных с ПДн граждан РК, файловые хранилища и резервные копии размещаются **на территории Республики Казахстан**. Единственное согласованное заказчиком исключение из контура РК — обращения к внешним провайдерам языковых моделей, в которые передаются только деперсонализированные учебные данные (FR-AI-02). Иных исключений нет. |
| PD-02 | Обработка ПДн несовершеннолетних осуществляется при наличии зафиксированного согласия законного представителя. Поскольку кабинет родителя не разрабатывается, согласие собирается школой вне платформы, а в системе фиксируется факт, дата, версия текста и способ получения (см. FR-CORE-10). Платформа предоставляет школе шаблон согласия на русском и казахском языках и отчёт о том, по каким ученикам согласие отсутствует. |
...
| PD-04 | Персональные данные учеников **не передаются** сторонним сервисам. Внешним провайдерам языковых моделей передаётся деперсонализированный учебный контент (текст задания и ответа ученика без идентифицирующих данных) — это единственный поток данных за пределы контура РК, он обеспечивается FR-AI-02 и FR-AI-05a и подлежит отдельной проверке при приёмке. |
...
| PD-09 | **РЕШЕНО заказчиком:** используется вариант (а) — внешние провайдеры LLM. Обязательные условия: договор на обработку с провайдером, отключение использования переданных данных для обучения моделей (opt-out), передача только деперсонализированных данных (FR-AI-02), журналирование всех обращений (FR-AI-07). Развёртывание собственной модели в контуре РК в объём проекта не входит (FR-AI-12 переведено в статус C). |
...
| INT-04 | M | Внешние провайдеры LLM | Через ИИ-шлюз, с абстракцией провайдера, резервным провайдером (FR-AI-08) и возможностью замены без изменения прикладного кода. Обязателен договор на обработку с отключением использования данных для обучения моделей (PD-09). |
| INT-05 | S | SMS-шлюз | Восстановление доступа и служебные уведомления учителям и администрации. Рассылки родителям не предусмотрены (FR-CORE-09). |
...
9. Тексты публичного сайта и политики конфиденциальности приведены в соответствие с FR-WEB-05 (формулировки о банке заданий PISA и о передаче данных в ИИ-сервисы).
10. Передана документация: OpenAPI, схема БД, ADR, руководства администратора школы, учителя и ученика (RU/KZ), runbook эксплуатации.
11. Проведено обучение: не менее 2 сессий для учителей и 1 сессии для администраторов школ-пилотов.
...
| **4. Масштабирование** (3 мес.) | Дашборд УоО, интеграции с журналами и SSO, нагрузочная оптимизация, наполнение контентом 1–11 | Готовность к массовому внедрению |
...
|---|---|---|---|
| R-1 | Формулировка на публичном сайте «платформа не передаёт данные детей сторонним сервисам» при использовании внешних LLM | Высокое (репутация, комплаенс) | Деперсонализация (FR-AI-02) и маскирование ПДн во вводе (FR-AI-05a); блокирующая проверка при приёмке (п. 12.2.7); **обязательное уточнение формулировки на сайте и в политике конфиденциальности** — см. FR-WEB-05 |
...
| R-6 | Неопределённость спецификации PISA-2029 и отсутствие лицензированных материалов OECD | Среднее | Банк формируется из тренировочных заданий собственной разработки по открытым рамкам OECD (FR-PSA-01); архитектура допускает переразметку метаданных без переписывания заданий; трудоёмкость авторской разработки заданий заложена в этап 2 |
...
| R-11 | Собственные тренировочные задания уступают по качеству и калибровке подлинным материалам PISA — прогноз готовности окажется неточным | Среднее (доверие администрации) | Экспертная рецензия каждого задания методистом-предметником; пилотная калибровка сложности на выборке ≥ 200 учеников; в отчётах прогноз баллов подаётся как модельная оценка с указанием выборки (FR-PSA-25, FR-ANL-07) |
| R-12 | Отсутствие кабинета родителя снижает вовлечённость семьи и создаёт нагрузку на учителя по информированию | Низкое | Выгружаемые отчёты для родительских собраний (приложение Б), сводка по классу в 1 клик; при появлении спроса кабинет родителя возвращается в план как отдельная инициатива |

## Решённые вопросы

| № | Вопрос | Решение заказчика | Где зафиксировано |
|---|---|---|---|
| Q-1 | Архитектура ИИ-контура: внешние провайдеры LLM или self-hosted модель в контуре РК | Внешние провайдеры LLM. Контур РК соблюдается для всех данных **за исключением** обращений к моделям, куда уходят только деперсонализированные учебные данные | PD-01, PD-04, PD-09, FR-AI-02, FR-AI-05a, FR-AI-12 |
| Q-4 | Объём кабинета родителя в v1 | Кабинет родителя не разрабатывается ни в каком объёме. У родителей нет учётных записей и уведомлений; согласия собираются школой вне платформы | FR-CORE-09, FR-NTF-04, FR-CORE-10, PD-02 |
| Q-6 | Источник и правовой статус заданий банка CRAFT PISA | Только тренировочные задания собственной разработки по открытым рамкам OECD. Лицензирование материалов OECD не производится | Раздел 1.3 части 1, FR-PSA-01, R-6, R-11 |
| Q-7 | Backend-стек: .NET или Node.js — с учётом доступности кадров у заказчика | .NET 10 (LTS) + ASP.NET Core, модульный монолит. Node.js + NestJS не рассматривается. Более узкий рынок .NET-кадров в РК принят как осознанная цена | [ADR-0001](../adr/0001-backend-stack-dotnet.md), раздел 5.3 части 1, [план, раздел 2.2](../plan/01-bootstrap-platform.md) |
...
| Портфолио ученика | Ученик, учитель | PDF, ссылка | Артефакты, описания, оценки. PDF пригоден для передачи родителям силами школы |
...
| «Защищённый учебный контур без риска утечки данных» | FR-AI-01, FR-AI-02, FR-AI-05a, PD-01, PD-04, NFR-SEC-04 |
...
| «Архив реальных заданий PISA с автопроверкой» | FR-PSA-01, FR-PSA-04 — ⚠️ формулировку на сайте требуется скорректировать: банк состоит из тренировочных заданий собственной разработки, а не из подлинных материалов OECD (FR-WEB-05) |
...
| «Данные хранятся на территории РК» | PD-01 — с оговоркой об обращениях к внешним LLM |
| «Не передаём данные детей сторонним сервисам» | FR-AI-02, FR-AI-05a, PD-04, PD-08 — ⚠️ формулировку требуется уточнить: деперсонализированный учебный текст передаётся внешнему провайдеру LLM (FR-WEB-05) |
...
- **Must (v1.0):** FR-CORE-01…04, 06…08, 10, 11, 13; FR-RBAC-01…05; FR-CNT-01…06, 08;
  ...
  FR-ASM-01…05; FR-SIM-01…05; FR-AI-01…05a, 06…08, 10, 11; FR-ANL-01…05, 07; FR-NTF-01…03, 05;
  FR-WEB-01…03, 05; FR-INT-01…06; все NFR и PD.
- **Should (v1.1):** FR-CORE-05, 12; FR-CNT-07; FR-CMS-06, 07; FR-MTX-07, 19, 23;
  FR-BLD-08, 11; FR-PSA-07, 25; FR-ASM-06, 07; FR-AI-09; FR-ANL-06;
  FR-WEB-04; INT-02, 03, 05, 08.
- **Could (backlog):** FR-AI-12 (архитектурная готовность к self-hosted модели); INT-06;
  нативные приложения; маркетплейс контента; прокторинг.
- **Исключено решением заказчика:** FR-CORE-09 и FR-NTF-04 (кабинет родителя и уведомления
  родителям); лицензирование и публикация подлинных заданий OECD; развёртывание собственной
  языковой модели в контуре РК.
