...
> **Первый рабочий срез (v0)** уточнён отдельно в
> [06-matrix-mvp0-kontent-i-zadania.md](06-matrix-mvp0-kontent-i-zadania.md):
> библиотека контента + автопроверяемые задания, без сессии урока и без
> симуляторов. Ниже — полная целевая раскладка, из которой v0 использует
> только часть `Content` и `Learning/StudentFlow` (§5 того документа).
...
> **Первый рабочий срез (v0)** уточнён отдельно в
> [07-pisa-mvp0-trenazhery.md](07-pisa-mvp0-trenazhery.md): только тренажёры
> трёх направлений, без срезов и без управленческого дашборда. Ниже — полная
> целевая раскладка, из которой v0 использует только часть `ItemBank` и
> `Trainers` (§4 того документа).
...
> Минимальные функции создания организации/класса/учётной записи и
> минимальная форма создания контента для методиста платформы (закрывает
> открытые вопросы О1 планов 06 и 07) описаны отдельно в
> [08-platform-admin-mvp0.md](08-platform-admin-mvp0.md).
