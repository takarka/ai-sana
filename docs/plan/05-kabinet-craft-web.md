# Проектирование кабинета CRAFT AI (`apps/learn` и `apps/admin`)

> **Имя файла историческое.** Документ написан, когда кабинет был одним
> приложением `craft-web`. 11.09.2026 оно разделено на `apps/admin` и
> `apps/learn` ([ADR-0005](../adr/0005-razdelenie-kabineta-na-admin-i-learn.md)),
> текст приведён в соответствие, а имя файла сохранено: переименование оборвало
> бы ссылки из десятка других документов ради косметики.

**Задача.** Спроектировать рабочий кабинет платформы — то, что живёт за
авторизацией и обслуживает все роли из ТЗ: ученик, учитель, администрация
...
разделение кабинета на два приложения; меняет §2 и §5 этого документа.
...
[08-platform-admin-mvp0.md](08-platform-admin-mvp0.md) — четвёртый контур
`platform` (не описан в §2 ниже, добавлен позже): школы, аккаунты и контент
для `superadmin` и методиста платформы, до появления школьного `org_admin`.

**Схема в HTML.** [05-kabinet-craft-web.html](05-kabinet-craft-web.html) — та же
структура наглядно: контуры школы с переключением, карта маршрутов, макеты
...
| Кабинет — два приложения: `apps/learn` (контуры школы) и `apps/admin` (контур `platform`); роли **внутри** каждого — маршрутизацией, а не отдельными сборками | [ADR-0005](../adr/0005-razdelenie-kabineta-na-admin-i-learn.md), [план 01 §2.1](01-bootstrap-platform.md) |
...
## 2. Четыре контура в двух приложениях

Роли из ТЗ (§3.1) сводятся к контурам — не по названию должности, а по
характеру работы: что человек делает, как часто и с какой плотностью экрана.
Три контура школы живут в `apps/learn`, четвёртый, платформенный
([план 08 §1](08-platform-admin-mvp0.md#1-новый-контур-platform)) — в
`apps/admin`.

```mermaid
flowchart LR
    subgraph Learn["apps/learn — один SPA, один бандл ядра"]
        AUTH["Вход<br/>/auth/*"]
        ...
        MANAGE["Контур MANAGE<br/>завуч, org_admin, методист,<br/>УоО<br/>таблицы и отчёты"]
    end
    subgraph Admin["apps/admin — отдельная сборка"]
        PAUTH["Вход<br/>/auth/*"]
        PLATFORM["Контур PLATFORM<br/>superadmin, методист платформы<br/>без организации-контекста"]
    end
    AUTH --> CTX
    ...
    PAUTH --> PLATFORM
    Admin -. "общие libs: ui, токены, i18n, data-access, api-client" .-> Learn
```

**Почему контуры школы не разделены на приложения.** Учитель школы одновременно
бывает классным руководителем и методистом (FR-RBAC-01 прямо требует несколько
ролей в разных областях у одной учётной записи); завуч ведёт свои уроки.
Разделение на сборки означало бы три логина, три сессии и дубль дизайн-системы
ради разной навигации. Контур — это layout и меню, а не приложение.

**Почему `platform` всё-таки вынесен.** Там этого совмещения нет: у
`superadmin` и методиста платформы нет организации-контекста, они не делят с
ролями школы ни одного экрана, а их учётные записи принадлежат оператору
платформы, а не школе. Полный разбор границы и её цены —
...
layout «шапка + сайдбар» для всех даст плохо всем.

| Контур | Приложение | Роли | Layout | Плотность | Поверхность |
|---|---|---|---|---|---|
| `learn` | `learn` | `student` | Один столбец, шаги во всю ширину, минимум хрома | `junior` (1–4 кл.) / `comfortable` | paper |
| `teach` | `learn` | `teacher`, `homeroom` | Рабочая область + правая панель «кому помочь» | `compact` | paper, дашборд урока — ink ([план 03 §3.2](03-design-system.md)) |
| `manage` | `learn` | `principal`, `org_admin`, `methodist`, `district`, `support` | Сайдбар-навигация + таблицы | `compact` | paper |
| `platform` | `admin` | `superadmin`, методист платформы (`author`) | Сайдбар-навигация + таблицы, без выбора организации | `compact` | paper |
...
### 3.5. Контур `platform` (`apps/admin`)

Живёт в отдельном приложении, маршруты `/platform/**`. Карта экранов не
дублируется здесь — она в [плане 08 §3](08-platform-admin-mvp0.md#3-экраны-и-маршруты),
а разбивка работ по ним — в [плане 09 §4](09-admin-panel-razrabotka.md#4-frontend).
Отличие от контуров школы одно и принципиальное: организация-контекст
отсутствует, поэтому `contextGuard` к этим маршрутам не применяется (§4).
...
platform/front/
├── apps/
│   ├── learn/                          # контуры школы: bootstrap, роуты верхнего уровня, конфиг
│   ├── learn-e2e/
│   ├── admin/                          # контур platform (ADR-0005): то же, своя сборка
│   ├── admin-e2e/
...
| `type:app` → только `type:feature`, `type:ui`, `type:data-access`, `type:util` | Уже настроено; тег носят оба приложения — `admin` и `learn` |
| Импорт `shared/api-client` вне `*/data-access` и `shared/data-access` запрещён | Компоненты не ходят в сеть напрямую |
| `admin` не импортирует `matrix/feature-lesson`, `learn` не импортирует `org/feature-*` платформенных экранов | Приложения не знают о чужих контурах; общее лежит в `libs/shared` и в доменных `data-access` |
...
≤ 3 с. Бюджеты по умолчанию в `apps/learn/project.json` и `apps/admin/project.json`
(`initial`: warning 500 КБ, error 1 МБ) измеряют **несжатый** размер и к
требованию отношения не имеют.
...
| e2e (Playwright) | `learn-e2e` | Вход учеником 1 класса по PIN; урок целиком; дашборд обновляется ≤ 5 с; срез с таймером и автосдачей; выгрузка отчёта |
| e2e (Playwright) | `admin-e2e` | Создать школу → класс → учителя → войти им; импорт класса из XLSX с битой строкой ([план 09 §6](09-admin-panel-razrabotka.md#6-тестирование)) |
...
Обновлено решением этой сессии после сужения MATRIX ([план 06](06-matrix-mvp0-kontent-i-zadania.md))
и PISA ([план 07](07-pisa-mvp0-trenazhery.md)) до первого среза и появления
платформенной админ-панели ([план 08](08-platform-admin-mvp0.md)):
**последовательность сборки экранов больше не совпадает с этапами ТЗ §13.1**
(они остаются верным ориентиром для содержания полной версии, см. §16.2, но не
для того, что собирается первым). Каждый шаг заканчивается работающим
экраном, а не «слоем».

### 16.1. Очередь v0

Админ-панель — первый шаг не по формальному приоритету ТЗ, а потому что без
неё нечего показывать ученику и учителю: до неё не существует ни одной школы,
класса, учётной записи или урока.

| Шаг | Содержание | Зависимость от backend | Требования |
|---|---|---|---|
| 1 | Каркас `apps/admin`: shell контура `platform`, меню, тема/плотность/язык, страницы ошибок | нет (моки) | §1, §11, §12 |
| 2 | Вход: логин, интерцепторы, refresh, guard'ы — в общих `libs/shared`, подключается к `admin` | `Identity` (узкий срез) | FR-CORE-03 |
| 3 | **Платформенная админ-панель** ([план 08](08-platform-admin-mvp0.md)): создать школу, класс, учётные записи — вручную и XLSX | `Organizations` (узкий срез) | FR-CORE-01/02/08 |
...
Шаг 1 не блокируется backend: контракт берётся из OpenAPI-черновика, работа
идёт на сгенерированном клиенте с моками — единственное место, где моки
допустимы. Начиная с шага 2 каждый шаг требует реального API того среза
бэкенда, что в нём указан.

### 16.2. Дальше (после v0, порядок между строками не приоритизирован)

BUILDER решением этой сессии остаётся отложен целиком. Остальное — то, что
v0 сознательно сузил (см. §3 планов 06/07 и §«Что этот срез НЕ делает» плана 08).

| Содержание | Зависимость от backend | Требования |
|---|---|---|
| MATRIX: симуляторы, сессия урока и дашборд учителя в реальном времени, журнал F/P | `Learning`, realtime, `Assessment` | FR-SIM-01…05, FR-MTX-10…19, FR-ASM-01…05 |
| PISA: спецмодуль «Медиа- и ИИ-грамотность», адаптивный подбор, срезы, управленческий дашборд | `Pisa`, `Analytics` | FR-PSA-03/07/10…25 |
| BUILDER: тренажёр промптов, песочница, портфолио, рубрики | `AiGateway`, `Builder` | FR-BLD-01…10 |
| Школьный `org_admin`: тонкая раздача ролей внутри школы, согласия, 2FA, лицензии, аудит | `Organizations`, `AiGateway` | FR-CORE-06/10/12 |
| Полноценная авторская студия (визуальный редактор, рецензия) и дашборд УоО | `Content`, `Analytics` | FR-CMS-01…07, FR-ANL-04 |
...
| K-4 | Zoneless change detection (Angular 20) для кабинета? | Да, но решением после шага 1: каркас проверяет, что CDK-компоненты и тесты живут без zone.js. Сейчас `admin` и `learn` собраны с `zone.js`, как и лендинг. Проверять на `admin` — он собирается первым |
