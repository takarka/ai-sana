...
public sealed class ModuleBoundaryTests
{
    ...
    [Fact]
    public void Organizations_НеДолженСсылатьсяНаРеализациюIdentity()
    {
        var referencedAssemblyNames = OrganizationsAssembly.GetReferencedAssemblies().Select(a => a.Name).ToArray();

        Assert.DoesNotContain(IdentityAssembly.GetName().Name, referencedAssemblyNames);
    }
}
